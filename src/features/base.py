...
class BasePreprocessor:
    ...
    def preprocess_train(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.cfg.models.name == "tabnet":
            df = self._standard_train_features(df)

        df = self._categorize_train_features(df)
        df = df.drop(columns=self.cfg.data.drop_features)
    ...
    def preprocess_test(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.cfg.models.name == "tabnet":
            df = self._standard_test_features(df)

        df = self._categorize_test_features(df)
    ...
    def _categorize_train_features(self, train: pd.DataFrame) -> pd.DataFrame:
        ...
        for cat_feature in tqdm(self.cfg.data.cat_features, desc="Encoding train data", leave=False):
            le = LabelEncoder()
    ...
    def _categorize_test_features(self, test: pd.DataFrame) -> pd.DataFrame:
        ...
        for cat_feature in tqdm(self.cfg.data.cat_features, desc="Encoding test data", leave=False):
